...
function getCounterpart(booking: Booking, viewer: User | null): { name: string; label: string } {
  if (!viewer) {
    return { name: booking.artist.name, label: "Creative" };
  }
  ...
  return (
    <main className={styles.page}>
      <header className={styles.header}>
        <div>
          <p className={styles.kicker}>Live project lifecycle</p>
          <h1>My projects</h1>
          <p className={styles.subtle}>
            Projects are scoped to your account and update as creatives confirm,
            schedules progress, and work is completed.
          </p>
        </div>
        <Link className={styles.primaryBtn} href="/projects/new">
          New project
        </Link>
      </header>
      ...
      {loading ? <p className={styles.subtle}>Loading projects...</p> : null}
      ...
      {!loading && !error && bookings.length === 0 ? (
        <section className={styles.emptyState}>
          ...
          <p className={styles.subtle}>
            Your project requests and confirmed work will appear here.
          </p>
          <Link className={styles.primaryBtn} href="/projects/new">
            Create a project
          </Link>
              ...
              <div className={styles.cardActions}>
                {booking.availableActions?.includes("confirm") ? (
                  <button
                    ...
                    onClick={async () => {
                      ...
                      try {
                        ...
                        if (err instanceof ApiError) {
                          ...
                          setError("Unable to confirm the project right now.");
                        }
                      } finally {
                        setActingId(null);
                      }
                    ...
                    title={onboardingLocked ? "Finish onboarding to confirm projects." : undefined}
                ...
                {booking.availableActions?.includes("cancel") ? (
                  <button
                    ...
                    onClick={async () => {
                      ...
                      try {
                        ...
                        if (err instanceof ApiError) {
                          ...
                          setError("Unable to cancel the project right now.");
                        }
                      } finally {
                        setActingId(null);
                      }
                    ...
                    disabled={actingId === booking.id || onboardingLocked}
                    title={onboardingLocked ? "Finish onboarding to cancel projects." : undefined}
                ...
                {booking.availableActions?.includes("complete") ? (
                  <button
                    ...
                    onClick={async () => {
                      ...
                      try {
                        ...
                        if (err instanceof ApiError) {
                          ...
                          setError("Unable to complete the project right now.");
                        }
                      } finally {
                        setActingId(null);
                      }
                    ...
                    disabled={actingId === booking.id || onboardingLocked}
                    title={onboardingLocked ? "Finish onboarding to complete projects." : undefined}
                    className={styles.actionPrimary}
                  >
                    Complete
                  </button>
                ...
                <Link href={`/projects/${booking.id}`} className={styles.actionGhost}>
                  View
                </Link>
              </div>
            </article>
