...
export function ArtistProfileActions({
  ...
  const bookNow = async () => {
    if (!artistId) {
      setError("This creative profile is not ready for project requests yet.");
      return;
    }
    ...
    try {
      ...
      router.push(`/projects/new?artistId=${encodeURIComponent(artistId)}`);
    } catch {
      router.push(
        buildLoginRedirect(`/projects/new?artistId=${encodeURIComponent(artistId)}`),
      );
    }
  };
  ...
  const messageArtist = async () => {
    if (!artistId) {
      setError("This creative profile is not ready for messaging yet.");
      return;
    }
    ...
    try {
      ...
      router.push(
        buildLoginRedirect(`/creatives/${encodeURIComponent(artistSlug)}?message=1`),
      );
      setIsMessaging(false);
      return;
    }
  ...
  if (!authed) {
    return (
      <>
        ...
        <Link href="/join" className={messageClassName}>
          I&apos;m a Creative
        </Link>
      </>
    );
